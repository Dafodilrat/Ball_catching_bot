...
class yahboomcar_driver(Node):
	def __init__(self, name):
		...
		global car_type_dic
		self.RA2DE = 180 / pi
		self.car = Rosmaster(com="/dev/ttyUSB0")
		...
		#create timer
		self.timer = self.create_timer(0.25, self.pub_data)
	...
	# def set_motor_vel_callback(self,msg):
	...
	# 	L = 0.125   # Distance from center to wheel along Y-axis
	# 	W = 0.115   # Distance from center to wheel along X-axis
	# 	R = 0.024   # Wheel radius

	# 	M1 = (msg.y + msg.x - (L + W) * msg.z) / R  # Left Front Wheel
	# 	M2 = (msg.y - msg.x - (L + W) * msg.z) / R  # Left Back Wheel
	# 	M3 = (msg.y + msg.x + (L + W) * msg.z) / R  # Right Back Wheel
	# 	M4 = (msg.y - msg.x + (L + W) * msg.z) / R  # Right Front Wheel
		
	# 	self.car.set_motor(M1, M2, M3, M4)
